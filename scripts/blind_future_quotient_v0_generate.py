...
PREREG="53a4f0932653c72295fc4e4d55308de754ff8913"
...
def build():
    ...
    hidden_blocks=[]
    for obs in (0,1):
        hidden_blocks.append(sorted(i for i,x in enumerate(latent) if bits(x)[1]==obs))
    exact=[]; behavioral=[]
    for i,a in enumerate(ids):
        for b in ids[i+1:]:
            p=sorted([a,b]); sa,sb=id_to_sem[a],id_to_sem[b]
            if exact_commute(sa,sb): exact.append(p)
            if behavioral_commute(sa,sb): behavioral.append(p)
    hidden={
      "schema":"metatron.blind.future-quotient.v0.hidden",
      "seed_hex":f"{seed:064x}",
      "public_state_to_latent":latent,
      "action_id_to_semantic":id_to_sem,
      "hidden_future_blocks":sorted(hidden_blocks),
    ...
    hb=cb(hidden)
    public={
      "schema":"metatron.blind.future-quotient.v0.public",
