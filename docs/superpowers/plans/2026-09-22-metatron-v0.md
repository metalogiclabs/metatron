...
**Produces:** exact-head CI gate plus an external evidence-closure attestation artifact, avoiding self-referential commit metadata.
...
jobs:
  v0:
    ...
    steps:
      ...
      - name: Write evidence closure attestation
        run: |
          python - <<'PY'
          import json
          import os
          ...
          record_path = Path("evidence/qualified-runs/metatron-v0.json")
          qualified_sha = None
          if record_path.exists():
              record = json.loads(record_path.read_text())
              qualified_sha = record.get("qualified_implementation_sha")

          attestation = {
              "schema_version": 1,
              "evidence_closure_sha": os.environ["GITHUB_SHA"],
              "workflow_run": {
              ...
              "qualified_implementation_sha": qualified_sha
          }
          Path("/tmp/metatron-v0-evidence-closure.json").write_text(
              json.dumps(attestation, indent=2, sort_keys=True) + "\n"
          )
          PY

      - name: Upload evidence closure attestation
        uses: actions/upload-artifact@v4
        with:
          name: metatron-v0-evidence-closure
          path: /tmp/metatron-v0-evidence-closure.json
          if-no-files-found: error
~~~

- [ ] **Step 2: Verify exact workflow commands locally**

Run manifest checker, both Lake builds, FixtureTest, placeholder scan, and full unittest discovery. The attestation step itself is CI-specific because it relies on GITHUB_SHA and GITHUB_RUN_ID.

Expected: all local commands exit 0.

- [ ] **Step 3: Commit**

~~~bash
git add .github/workflows/metatron-v0.yml
git commit -m "ci: add Metatron V0 qualification and closure gate"
~~~

### Task 12: Remote qualification and non-self-referential evidence closure

**Files:** create evidence/qualified-runs/metatron-v0.json; modify README.md and spec/CLAIM_BOUNDARY.md

**Produces:** a committed record of the qualified implementation head, plus an externally attested evidence-closure head from the final workflow artifact.

- [ ] **Step 1: Push the implementation head**
...
Find the Metatron V0 run whose head_sha exactly equals:
...
Call this exact commit QUALIFIED_IMPLEMENTATION_SHA and this run QUALIFIED_IMPLEMENTATION_RUN.

- [ ] **Step 2: Verify required implementation-run steps**

The exact implementation run must show success for:
...
Write evidence closure attestation
Upload evidence closure attestation
~~~

A missing, skipped, cancelled, or failed required step means the implementation is not qualified.

- [ ] **Step 3: Commit the qualified implementation identity**

After obtaining RUN_ID, QUALIFIED_IMPLEMENTATION_SHA, and RUN_URL, execute:

~~~bash
python - "$RUN_ID" "$QUALIFIED_IMPLEMENTATION_SHA" "$RUN_URL" <<'PY'
import json
import sys
from pathlib import Path

run_id = int(sys.argv[1])
qualified_sha = sys.argv[2]
run_url = sys.argv[3]
...
record = {
    ...
    "evidence_closure": {
        "mode": "external_workflow_artifact",
        "artifact_name": "metatron-v0-evidence-closure",
        "note": (
            "The evidence-closure commit cannot contain its own future SHA/run. "
            "Its exact SHA is attested externally by the final workflow artifact."
        )
    },
    "claim": (
        "Finite proof-carrying developmental lifecycle: exact obstruction, "
        "issued residual warrant, verifier-bound capability promotion, earned "
        "relation, query-relative compression, deterministic restart, "
        "revocation, and regrowth."
    ),
    "excluded_claims": [
        "GeneratedCongruence(Omega_earned) = PathBehEq",
        "residualAdjoin_strictLyapunov",
        "nonlinear support/recombination",
        "dependency-driven Flash reclosure",
        "full (state, value, warrant, residual) execution interface",
        "unrestricted self-improvement",
        "infinite productivity"
    ]
}
...
- [ ] **Step 4: Update public claim status without implying self-reference**
...
Do not remove any excluded claim.

- [ ] **Step 5: Commit the evidence closure**

~~~bash
git add evidence/qualified-runs/metatron-v0.json README.md spec/CLAIM_BOUNDARY.md
git commit -m "evidence: record qualified Metatron V0 implementation"
~~~

Call this new commit EVIDENCE_CLOSURE_SHA.

- [ ] **Step 6: Re-run complete local qualification on the evidence-closure commit**
...
- [ ] **Step 7: Push, require final green run, and verify external attestation**
...
Find the Metatron V0 run whose head_sha equals EVIDENCE_CLOSURE_SHA. Require conclusion success.

Download the run artifact named:

~~~text
metatron-v0-evidence-closure
~~~

and verify its JSON contains:

~~~text
evidence_closure_sha == EVIDENCE_CLOSURE_SHA
qualified_implementation_sha == QUALIFIED_IMPLEMENTATION_SHA
workflow_run.conclusion == "success"
~~~

The final user report must cite:
- QUALIFIED_IMPLEMENTATION_SHA and its successful run;
- EVIDENCE_CLOSURE_SHA;
- the final evidence-closure run and attestation artifact.

The committed JSON intentionally records only the qualified implementation identity; the final evidence-closure identity is external to avoid impossible self-reference.
...
- Supplied capability/relation certificates are accepted only when equal to their persisted store records.
- Free extension requires an issued residual certificate whose target, closure, and authority digests still match.
- Candidate verification derives its target from the issued residual warrant; there is no caller-supplied expected table.
- Every QUALIFIED external dependency has a successful run URL and exact blob/artifact identifier.
- The committed qualification record names only the qualified implementation head; final evidence closure is externally attested.
- V0 claim text explicitly defers nonlinear support/recombination, Flash reclosure, and the full execution-result interface.
